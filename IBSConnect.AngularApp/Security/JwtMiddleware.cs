...
public class JwtMiddleware
{
    ...
    private async Task AttachUserToContextAsync(HttpContext context, string token)
    {
        try
        {
            ...
            var userClaims = jwtToken.Claims.GetClaimsAs<UserClaims>();

            var identity = new IBSIdentity(
                userClaims.Id, 
                userClaims.Username, 
                $"{userClaims.FirstName} {userClaims.MiddleName} {userClaims.LastName}", 
                userClaims.Role, 
                validatedToken.ValidTo, 
                jwtToken.Claims
            );

            context.User = new IBSPrincipal(identity);
        }
        catch
        {
